...
from application.main.utility.face_smooth_utility.detector import detect, smooth
...
def process_image(input_img, net):
    """
    ...
    detected_img, bboxes = detect.detect_face(net, input_img)
    # Smooth face and return steps
    output_img, roi_img, hsv_mask, smoothed_roi = smooth.smooth_face(input_img, bboxes)
    # Draw bboxes on output_img
    output_w_bboxes = draw_bboxes(output_img,bboxes)
    return (input_img, detected_img, roi_img, hsv_mask, 
            smoothed_roi, output_w_bboxes, output_img) ,bboxes
...
def draw_bboxes(output_img, bboxes):
    """
    Draw bounding boxes on an image.
    ...
    for i in range(len(bboxes)):
        ...
        cv2.rectangle(output_w_bboxes, 
                      top_left, 
                      btm_right, 
                      settings.SMOOTH_FACE_CONFIG.BBOX_COLOR, 
                      2)
    return output_w_bboxes        


def check_if_adding_bboxes(img_steps):
    """
