...
def smooth_face(detected_img, bboxes):
    """
    ...
    for box_num in range(len(bboxes)):
        ...
        hsv_mask = cv2.inRange(hsv_img, 
                               np.array(settings.SMOOTH_FACE_CONFIG.HSV_LOW), 
                               np.array(settings.SMOOTH_FACE_CONFIG.HSV_HIGH))
        ...
        blurred_img = cv2.bilateralFilter(roi_img, 
                                          settings.SMOOTH_FACE_CONFIG.DIAMETER, 
                                          settings.SMOOTH_FACE_CONFIG.SIGMA_1, 
                                          settings.SMOOTH_FACE_CONFIG.SIGMA_2)
        # Apply mask to image
