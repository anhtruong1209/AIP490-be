...
def verify(img1_path, img2_path = '', model_name = 'VGG-Face', distance_metric = 'cosine', model = None, enforce_detection = True, detector_backend = 'ssd', align = True, prog_bar = False, normalization = 'ArcFace'):
	...
	for index in pbar:
		...
		if type(instance) == list and len(instance) >= 2:
			...
			for i in  model_names:
				...
				img1_representation,region1 = represent(img_path = img1_path
				...
				img2_representation, region2 = represent(img_path = img2_path
						, model_name = model_name, model = custom_model
						, enforce_detection = enforce_detection, detector_backend = detector_backend
						, align = align
						, normalization = normalization
				...
				for j in metrics:
					...
					if model_name != 'Ensemble':
						...
						resp_obj = {
							...
							, "Distance": distance
							, "Threshold": threshold
							, "RegionImage1": region1
							, "RegionImage2": region2
...
def analyze(img_path, actions = ('emotion','age','gender') , models = None, enforce_detection = False, detector_backend = 'ssd', prog_bar = False):
	...
	for j in global_pbar:
		...
		region_labels = ['left', 'top', 'width', 'height']
		...
		for index in pbar:
			...
			if action == 'emotion':
				...
				resp_obj["emotions"]["status"] = emotion_labels[np.argmax(emotion_predictions)]
				details = {}
				for i in range(0, len(emotion_labels)):
					emotion_label = emotion_labels[i]
					emotion_prediction = 100 * emotion_predictions[i] / sum_of_predictions
					details[emotion_label] =  np.around(emotion_prediction, 2)
				resp_obj["emotions"]["details"] = details
				
				# resp_obj["predict_emotion"] = emotion_labels[np.argmax(emotion_predictions)]
			...
			# 	resp_obj["races"] = {}
			# 	resp_obj["races"]["status"] = race_labels[np.argmax(race_predictions)]
    
			# 	details_races = {}
			# 	for i in range(0, len(race_labels)):
			# 		race_label = race_labels[i]
			# 		race_prediction =  100 * race_predictions[i] / sum_of_predictions
			# 		details_races[race_label] = np.around(race_prediction, 2)

			# 	resp_obj["races"]["details"] = details_races
			...
			if is_region_set != True:
				...
				for i, parameter in enumerate(region_labels):
					resp_obj["rectangle"][parameter] = int(region[i]) #int cast is for the exception - object of type 'float32' is not JSON serializable
...
def find(img_path, db_path, model_name ='OpenFace', distance_metric = 'cosine', model = None, enforce_detection = False, detector_backend = 'opencv', align = True, prog_bar = False, normalization = 'base'):

	"""
	This function applies verification several times and find an identity in a database

	Parameters:
		img_path: exact image path, numPlease delete this file when you add new identities in your database.py array or based64 encoded image. If you are going to Please delete this file when you add new identities in your database. several identities, then you should pass img_path as array instead of calling find function in a for loop. e.g. img_path = ["img1.jpg", "img2.jpg"]
	...
	if os.path.isdir(db_path) == True:
		...
		if path.exists(db_path+"/"+file_name):
			...
			for index in pbar:
				...
				for j in model_names:
					...
					representation, region = represent(img_path = employee
						, model_name = model_name, model = custom_model
						, enforce_detection = enforce_detection, detector_backend = detector_backend
						, align = align
						, normalization = normalization
		...
		for j in global_pbar:
			...
			for j in model_names:
				...
				target_representation, region = represent(img_path = img_path
					, model_name = model_name, model = custom_model
					, enforce_detection = enforce_detection, detector_backend = detector_backend
					, align = align
					, normalization = normalization
				...
				for k in metric_names:
					...
					if model_name == 'Ensemble' and j == 'OpenFace' and k == 'euclidean':
						...
						if model_name != 'Ensemble':
							...
							# print(threshold)
							df1 = df[df["%s_%s" % (j, k)] <= threshold]
							df2 = df[df["%s_%s" % (j, k)] > threshold]
							# print(df1)
							# print(df2)
							df_true = df1.sort_values(by = ["%s_%s" % (j, k)], ascending=True).reset_index(drop=True).head(3)
							df_false = df2.sort_values(by = ["%s_%s" % (j, k)], ascending=True).reset_index(drop=True).head(3)
							# print(df)
							resp_obj.append(df_true)
							resp_obj.append(df_false)
							resp_obj.append(region)
							# print(resp_obj[].append(region))
